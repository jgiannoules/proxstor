...
public class DevicesResource {
    ...
    public Response getAllUserDevices() {
        if (UserDao.instance.getUser(userId) == null) {
            return Response.status(404).build();
        }     
        Collection<Device> devices = DeviceDao.instance.getAllUserDevices(userId);
        if (devices.isEmpty()) {
    ...
    public Response postUserDevice(Device in) {
        ProxStorDebug.println("postUserDevice");
        Device d = DeviceDao.instance.addUserDevice(userId, in);
        if (d == null) {
